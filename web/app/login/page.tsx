...
function LoginForm() {
  ...
  return (
    <div className="max-w-md mx-auto container-fluid py-12 sm:py-20 rise">
      ...
      <p className="text-mute text-center text-sm mb-8">Sign in to save and compare properties.</p>
      ...
      <div className="surface rounded-2xl p-6 sm:p-8 space-y-5">
        ...
        <form onSubmit={submit} className="space-y-4" noValidate>
          ...
          <div>
            ...
            <div className="relative">
              <input id="password" type={showPw ? "text" : "password"} value={password}
                ...
                required autoComplete="current-password" className="field pr-10" />
          ...
          <div className="flex justify-end -mt-2">
            <Link href="/forgot-password" className="text-xs text-mute hover:text-ember-500 transition-colors">Forgot password?</Link>
          </div>
          <button type="submit" disabled={busy || !email || !password} className="btn-primary w-full">
        ...
        <p className="text-center text-sm text-mute">
          No account?{" "}
          <Link href={`/signup?next=${encodeURIComponent(next)}`} className="link-underline brand-link font-medium">Sign up</Link>
        </p>
      </div>
    </div>
  );
